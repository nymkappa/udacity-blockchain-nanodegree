...
contract('FlightSuretyApp', async (accounts) =>
{
    ...
    it('[Passenger Withdraw] Passenger can withdraw any funds owed to them as a result of receiving credit for insurance payout / Insurance payouts are not sent directly to passenger’s wallet', async () => {
        ...
        let gazPrice = await web3.eth.getGasPrice()

		let currentBalance = await web3.eth.getBalance(config.customer1)
		currentBalance = BigNumber(currentBalance)
		let expectedBalance = currentBalance.plus(amount)

        await config.flightSuretyApp.testRefundCustomer(
            config.customer1, param, {from: config.owner})
		let tx = await config.flightSuretyApp.withdraw(amount, {from: config.customer1,
			gaz: 1})

		let fee = tx.receipt.gasUsed * gazPrice
		let newBalance = await web3.eth.getBalance(config.customer1)
		newBalance = BigNumber(newBalance)
		expectedBalance = expectedBalance.minus(BigNumber(fee))

		assert.strictEqual(newBalance.toString(), expectedBalance.toString(), 'Amount mistatch')
    })
})
